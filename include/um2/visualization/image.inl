...
Image::Image(const FaceVertexMesh<P, N, T, I> & mesh) noexcept
{
  ...
  //  try 4 by 4 grid
  partition.grid = RegularGrid<2, T>(bbox.minima, {nx / grid_size, ny / grid_size},
                                     {grid_size, grid_size});
  partition.children.resize(grid_size * grid_size);
  for (len_t i = 0; i < 16; i++) {
  ...
  for (len_t i_grid = 0; i_grid < grid_size; i_grid++) {
    for (len_t j_grid = 0; j_grid < grid_size; j_grid++) {
      ...
      Vec<8, I> face_indices = std::move(partition.getChild(i_grid, j_grid));
      //      remove all -1 in face_indices
      face_indices.erase(std::remove(face_indices.begin(), face_indices.end(), -1),
                         face_indices.end());
      for (len_t i = i_min; i < i_max; i++) {
        for (len_t j = j_min; j < j_max; j++) {
          for (len_t f_id = 0; f_id < face_indices.size(); f_id++) {
            auto face = mesh.get_face(face_indices[f_id]);
            if (face.contains(Vec<2, T>(bbox.minima[0] + j * width / nx,
